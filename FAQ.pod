...
and broke it into three parts: The interface specification (PSGI),
Reference server implementations (Plack::Server) and Standard APIs and
Tools (Plack).
...
For web application frameworks that need more controls over the actual
raw URI (such as L<Catalyst>), we made C<REQUEST_URI> environment hash
key REQUIRED. The servers should set the undecoded (unparsed) original
URI, containing query strings to this key.
