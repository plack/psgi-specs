...
The input stream in C<psgi.input> is an IO::Handle-like object which streams the raw HTTP POST or PUT data. If it is a file handle then it MUST be opened in binary mode. The input stream MUST respond to C<read> and MAY implment C<seek>.
...
If the same key name appears multiple times in an array ref, those header lines MUST be sent to the client (e.g. multiple C<Set-Cookie> lines).
...
Implementors SHOULD check if the body is whether a glob reference or an object that implements C<getline>, and call C<getline> to iterate the body and call C<close> when done.
